...
$data=mysql_query("SELECT *FROM aminpost ORDER BY id ")or die (mysql_error());
   $countt = ($data? mysql_affected_rows():0);
...
<br>
 <button type="button" class="btn btn-default"><a href="send.php"><i class="fas fa-paper-plane"></i>&nbsp; Send request(<?php echo "<b style='color:red'>".$countt."</b>"; ?>)</a></button>
 <div class="dropdown">
...
if(empty($cont))
  ...
  echo" ";
    ...
    $sq1=mysql_query("INSERT INTO tmpost(connt) VALUES ('$cont')");
...
<button type="submit" name="submitt"style="margin-left:100px;width:90px;margin-top:3px;"onclick="vali()" class="btn btn-default" value="Submit"><i class="fas fa-paper-plane"></i>Send</button>
<br>
  ...
  $result = mysql_query("SELECT * FROM aminpost ");
 ...
 while($row=mysql_fetch_array($data))
     {
      ...
      echo "<td>";echo"<p style=' font-size: 16px;color:black; '>"; echo $row['connt']; echo"</p>";echo "</td>";
    //  echo "<td>"; echo"<a href='accepte='".$row["id"]."'>Accept</a>";echo"</td>";
      ?>
      <td><a href="deletesend.php?slide=<?php echo $row['id'];?>">Cancle</a>
      <?php
      echo "</tr>";
    }
